...
common_rows=first_row(ismember(first_row,last_row));
common_columns=first_column(ismember(first_column,last_column));
common_border=common_rows(ismember(common_rows,common_columns));
sum_common_elements=sum(common_border);
if sum_common_elements==0 
    last=('The system has not reached percolation'); 
    perc=0;
else
